...
<!-- field: permission.scripting, limit: 1000 -->

```text
The extension uses chrome.scripting only after a trusted user click on a ChatGPT conversation. It reads nonce-scoped capture state from an inactive disposable chatgpt.com tab. A marker-gated local document-start helper observes only the exact same-origin conversation response generated by ChatGPT itself. Liska never reads or serializes cookies, request-header values, account data, or unrelated responses. Small responses use a bounded inline transfer; larger responses use independently verified 512 KiB chunks in extension-private temporary storage before the disposable tab closes. No remote code is loaded or executed.
```

624 characters. Liska draft; not yet pasted to the Chrome Web Store dashboard.
