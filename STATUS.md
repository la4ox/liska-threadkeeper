...
- Earlier generic `capture-failed` results on project/custom-GPT and some ordinary chats were reproduced as an SPA sender-URL mismatch: Chrome retained the content script's original document path while the tab moved to another conversation. The bounded same-origin authorization fix is live-verified, but the prioritized heavy corpus still needs a fresh pass to distinguish any remaining provider/normalization failures from this resolved routing defect.
...
Keep PR #7 in draft. Publish the SPA authorization checkpoint only with
maintainer approval, then rerun the prioritized heavy ChatGPT corpus with the
fixed build and classify only any remaining specific stage failures. Continue
next with explicit branch/all-branches presentation and provider-specific
